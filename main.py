...
# --- إعدادات الواجهة السينمائية (نسخة Wilked) ---
app = Flask(__name__)
...
def home():
    ...
    <head>
        ...
        <style>
            ...
            /* الضباب الأسود والأبيض */
            .fog-bg { position: absolute; width: 100%; height: 100%; z-index: 1; opacity: 0.2; background: url('https://cdn.pixabay.com/photo/2016/11/21/15/34/fog-1845173_960_720.png'); background-size: cover; animation: fogMove 90s linear infinite; }
            ...
            p { color: #555; font-size: 1.2rem; margin-top: 10px; font-style: italic; }
            .online { margin-top: 30px; border: 1px solid #43b581; color: #43b581; padding: 10px 30px; border-radius: 50px; font-weight: bold; background: rgba(67, 181, 129, 0.1); }
        </style>
    </head>
    <body>
...
def run():
    # هذا السطر هو اللي بيحل مشكلة Not Found في ريلواي
...
@bot.event
async def on_ready():
    print(f'✅ Ráinbot is up! Dev: Wilked')

@bot.command(name="موقع")
async def dashboard(ctx):
    await ctx.message.delete()
    # هنا حط رابط ريلواي حقك اللي طلع لك
